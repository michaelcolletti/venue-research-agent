...
def lookup_zip():
    ...
    try:
        ...
        if not result:
            return jsonify({"success": False, "error": "Zip code not found"})
    ...
    try:
        ...
        for zip_result in nearby_zips:
            ...
            if city and city not in cities_map:
                cities_map[city] = {"city": city, "county": county}
...
def preview_config():
    ...
    try:
        ...
        return jsonify({"success": True, "config": toml_content})

    except Exception as e:
        return jsonify(
            {"success": False, "error": f"Error generating config: {str(e)}"}
        )
