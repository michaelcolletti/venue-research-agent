        ...
        if venue.get("name") and venue.get("name") != "Example Excluded Venue":
            try:
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?)
                """, (venue.get("name"), venue.get("city"), venue.get("reason"), 
                      venue.get("date_excluded"), venue.get("notes")))
            except:
                pass
...
def match_acts_to_venue(config: dict, venue_type: str, genres: list) -> list[str]:
    ...
    for act_key, act_data in acts.items():
        ...
        genre_match = any(g in genres_lower for g in act_genres) if genres_lower else False
...
def generate_weekly_report(config: dict) -> Path:
    ...
    if opportunity_list:
        ...
        for opp in opportunity_list:
            ...
            report_lines.append(f"### {priority_label} {otype.replace('_', ' ').title()}")
            if venue_name:
    ...
    for act_key, act_data in config.get("acts", {}).items():
        report_lines.append(f"- **{act_data.get('name', act_key)}**: {', '.join(act_data.get('genres', []))}")
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?)
    """, (report_date, week_start_str, week_end_str, str(report_path),
          new_venues, new_opportunities))
    conn.commit()
    conn.close()
...
def list_acts(config: dict):
    ...
    for act_key, act_data in acts.items():
        ...
        print(f"   Capacity: {act_data.get('min_capacity')}-{act_data.get('max_capacity')} (ideal: {act_data.get('ideal_capacity')})")
...
def export_venues(output_format: str = "json") -> Path:
    ...
    export_path = DATA_DIR / f"venues_export_{datetime.now().strftime('%Y%m%d')}.{output_format}"
...
def unexclude_venue(name: str, city: str):
    ...
    cursor.execute("DELETE FROM excluded_venues WHERE name = ? AND city = ?", (name, city))
...
def main():
    ...
    parser.add_argument("--daily", action="store_true", 
                       help="Run daily search routine")
    parser.add_argument("--weekly-report", action="store_true",
                       help="Generate weekly summary report")
    parser.add_argument("--list-acts", action="store_true",
                       help="List configured acts")
    parser.add_argument("--init-db", action="store_true",
                       help="Initialize/reset database")
    parser.add_argument("--add-venue", nargs=2, metavar=("NAME", "CITY"),
                       help="Manually add a venue")
    parser.add_argument("--export", choices=["json", "csv"],
                       help="Export venues to file")
    parser.add_argument("--search", type=str,
                       help="Run ad-hoc search query")
    parser.add_argument("--max-queries", type=int, default=20,
                       help="Max queries for daily search (default: 20)")
    parser.add_argument("--exclude", nargs=2, metavar=("NAME", "CITY"),
                       help="Exclude a venue from searches")
    parser.add_argument("--exclude-reason", type=str, default="no_response",
                       choices=["no_response", "bad_experience", "closed", "not_booking", "wrong_fit"],
                       help="Reason for exclusion")
    parser.add_argument("--exclude-notes", type=str,
                       help="Notes for exclusion")
    parser.add_argument("--unexclude", nargs=2, metavar=("NAME", "CITY"),
                       help="Remove a venue from exclusion list")
    parser.add_argument("--list-excluded", action="store_true",
                       help="List all excluded venues")
    ...
    if args.init_db:
        ...
        exclude_venue(args.exclude[0], args.exclude[1], args.exclude_reason, args.exclude_notes)
    elif args.unexclude:
